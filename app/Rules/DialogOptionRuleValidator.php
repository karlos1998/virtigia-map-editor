    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        foreach ($value as $key => $ruleData) {
            ...
            if ($key === DialogNodeOptionRule::ITEMS->value) {
                ...
                $key === DialogNodeOptionRule::TIME_AFTER->value ||
                $key === DialogNodeOptionRule::TIME_BEFORE->value
            ) {
                if (! is_string($ruleData['value']) || ! preg_match(self::TIME_PATTERN, $ruleData['value'])) {
                    $fail("Dla rule: {$key}, wartość musi być godziną w formacie HH:MM.");

                    return;
                }
            } elseif ($key === DialogNodeOptionRule::WEEKDAY->value) {
                if (
                    ! is_array($ruleData['value']) ||
                    $ruleData['value'] === [] ||
                    ! collect($ruleData['value'])->every(fn ($day) => is_int($day) && $day >= 1 && $day <= 7)
                ) {
                    $fail("Dla rule: {$key}, wartość musi być niepustą tablicą dni tygodnia od 1 do 7.");

                    return;
                }
            } elseif ($key === DialogNodeOptionRule::ACTIVE_PLAYERS_ON_MAP->value) {
