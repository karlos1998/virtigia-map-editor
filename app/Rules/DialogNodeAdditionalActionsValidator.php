...
class DialogNodeAdditionalActionsValidator implements ValidationRule
{
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        foreach ($value as $key => $ruleData) {
            ...
            if ($key === DialogNodeAdditionalAction::ADD_ITEMS->value) {
                ...
                if (! QuestStep::where('id', $ruleData['value'])->exists()) {
                    $fail("Dla rule: {$key}, krok questa o ID {$ruleData['value']} nie istnieje.");

                    return;
                }
            } elseif ($key === DialogNodeAdditionalAction::BLESSING->value) {
                $blessingValue = $ruleData['value'] ?? null;
                if (! is_numeric($blessingValue)) {
                    $fail("Dla rule: {$key}, wartość musi być liczbą (ID przedmiotu).");

                    return;
                }

                $exists = BaseItem::where('id', $blessingValue)->where('category', 'blessings')->exists();
                if (! $exists) {
                ...
                $outfitValue = $ruleData['value'] ?? null;
                if (! is_string($outfitValue) || empty(trim($outfitValue))) {
